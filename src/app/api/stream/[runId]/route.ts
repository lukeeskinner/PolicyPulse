...
export async function GET(
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const send = (event: SimEvent) => enqueue(`data: ${JSON.stringify(event)}\n\n`);
      const finish = () => {
      ...
      // 1. replay everything that has happened so far (synchronous = race-free)
      for (const e of snap.events) send(e);

      // 2. if the run already finished, close immediately after the backlog
