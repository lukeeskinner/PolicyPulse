    ...
    <div className="glass rounded-2xl p-5 flex flex-col gap-4">
      {policyModel && (
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-cyan-500/15 text-cyan-300 border border-cyan-500/30">
          ...
          <div className="mt-2">
            <div className="flex items-center justify-between text-[10px] text-slate-500 mb-1">
              ...
              <span>{fmtPct(policyModel.intensity)}</span>
            </div>
            <div className="h-1.5 rounded-full bg-slate-800 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-cyan-400 to-violet-500" style={{ width: fmtPct(policyModel.intensity) }} />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="border-t border-slate-800/80 pt-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h4 className="text-xs font-semibold text-slate-200">Ingested sources</h4>
        ...
        <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
          ...
          {sources.map((s, i) => (
            <div key={i} className="flex items-start gap-2 animate-[pp-pop_0.25s_ease]">
              <span className="mt-0.5 text-cyan-400">{SOURCE_ICON[s.kind]}</span>
